...
{
    public partial class frmSalidaMedicamento : Form
    {
        ...
        public frmSalidaMedicamento()
        {
            InitializeComponent();
            conexion = new Conexion();
            adapterSalida = new SqlDataAdapter("spSalidaMedSelect", conexion.ObtenerConexion());
            adapterSalida.SelectCommand.CommandType = CommandType.StoredProcedure;
            tablaSalida = new DataTable();

            adapterDetalle = new SqlDataAdapter("spSalidaDetalleMedSelect", conexion.ObtenerConexion());
            ...
            toolTip1.SetToolTip(btnGuardar, "Guardar registro de Salida de medicamento");
            toolTip1.SetToolTip(btnGuardarDet, "Guardar registro de los detalles de la salida de medicamento");
            toolTip1.SetToolTip(btnEditar, "Editar registro de Salida o de los detalles de la salida de medicamento");
            toolTip1.SetToolTip(btnEditarDet, "Editar registro de los detalles de la salida de medicamento");
            toolTip1.SetToolTip(btnLimpiar, "Limpiar todos los campos");
            toolTip1.SetToolTip(btnEliminar, "Eliminar registro de Salida y los detalles de la salida de medicamento");
            toolTip1.SetToolTip(btnEliminarDet, "Eliminar registro de los detalles de la salida de medicamento");
            toolTip1.SetToolTip(btnSeleccionar, "Seleccionar un articulo/medicamento de las existencias");
            ...
            toolTip1.SetToolTip(txtCantidad, "Ingresar la cantidad de medicamento");
            toolTip1.SetToolTip(txtPrecio, "Ingresar el precio del medicamento");
        ...
        private void frmSalidaMedicamento_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            if (ValidarCamposSalida())
            {
                if (MessageBox.Show("¿Desea guardar el registro de la salida?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        using (SqlCommand cmdSalida = new SqlCommand("spInsertSalidamedicamento", con))
                        {
                ...
                if (MessageBox.Show("¿Desea editar el registro seleccionado?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        if (dgvSalida.SelectedRows.Count > 0)
                        {
                            ...
                            SqlConnection con = conexion.ObtenerConexion();
                            using (SqlCommand cmdSalida = new SqlCommand("spSalidaMedicamentoUpdate", con))
                            {
                                cmdSalida.CommandType = CommandType.StoredProcedure;
                ...
                if (MessageBox.Show("¿Desea editar el registro seleccionado?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        if (dgvDetalle.SelectedRows.Count > 0)
                        {
                            ...
                            SqlConnection con = conexion.ObtenerConexion();
                            using (SqlCommand cmdSalidaDet = new SqlCommand("spSalidaDetalleMedUpdate", con))
                            {
                                cmdSalidaDet.CommandType = CommandType.StoredProcedure;
        ...
        private void btnSeleccionar_Click(object sender, EventArgs e)
        {
            try
            {
                //Pasamos la referencia de frmSalidamedicamento al constructor de Vista_ArticuloCon y el parametro de conexion
                Vista_ArticuloMed frm = new Vista_ArticuloMed(conexion.ObtenerConexion(), this);
                frm.ShowDialog();
            }
            catch (Exception ex)
            {
            ...
            if (MessageBox.Show("¿Desea eliminar la Salida y sus respectivos detalles?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                try
                {
                    if (dgvSalida.SelectedRows.Count > 0)
                    {
                        int SalidaID = Convert.ToInt32(dgvSalida.SelectedRows[0].Cells["SalidaID"].Value);
                        SqlConnection con = conexion.ObtenerConexion();
                        using (SqlCommand cmdSalida = new SqlCommand("spSalidaMedDesactivarDetalles", con))
                        {
                            cmdSalida.CommandType = CommandType.StoredProcedure;
                            cmdSalida.Parameters.AddWithValue("@salidaid", SalidaID);
                            cmdSalida.ExecuteNonQuery();
        ...
        private void btnEliminarDet_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("¿Desea eliminar el detalle?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    {
                        int SalidaID = Convert.ToInt32(dgvDetalle.SelectedRows[0].Cells["SalidaID"].Value);
                        int ArticuloID = Convert.ToInt32(dgvDetalle.SelectedRows[0].Cells["ArticuloMedID"].Value);
                        SqlConnection con = conexion.ObtenerConexion();
                        using (SqlCommand cmdSalidaDet = new SqlCommand("spSalidaDetallesMedDesactivar", con))
                        {
                            cmdSalidaDet.CommandType = CommandType.StoredProcedure;
                            cmdSalidaDet.Parameters.AddWithValue("@salidaid", SalidaID);
                            cmdSalidaDet.Parameters.AddWithValue("@articuloid", ArticuloID);
        ...
        private void dgvDetalle_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (dgvDetalle.SelectedRows.Count > 0)
                {
                    ...
                    txtSalidaID.Text = row.Cells["SalidaID"].Value.ToString();
                    txtArticuloID.Text = row.Cells["ArticuloMedID"].Value.ToString();
        ...
        private void dgvSalida_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                try
                {
                    ...
                    SqlConnection con = conexion.ObtenerConexion();
                    using (SqlCommand cmd = new SqlCommand("spObtenerDetallesSalidaMed", con))
                    {
        ...
        private void CargarDetalles()
        {
            ...
            {
                SqlConnection con = conexion.ObtenerConexion();
                using (SqlCommand cmd = new SqlCommand("spSalidaDetalleMedSelect", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                    DataTable tablaDetalles = new DataTable();
